            ...
            >
              ...
              >
                  ...
                  {Object.entries(profile.stats).map(([key, value], i) => (
                    <div key={key} className="flex items-center justify-between text-sm">
                      <span className="text-gray-600 dark:text-gray-400 flex items-center gap-1">
                        {i === 0 && <FaTrophy className="w-3 h-3 text-yellow-500" />}
                        {i === 1 && <FaMedal className="w-3 h-3 text-blue-500" />}
                        {i === 2 && <FaFire className="w-3 h-3 text-orange-500" />}
                        {key.split(/(?=[A-Z])/).join(' ')}:
                      </span>
                ...
                <motion.a
                  ...
                  className={`block w-full py-2 text-center rounded-lg text-white text-sm font-medium
                            bg-gradient-to-r ${profile.color} hover:opacity-90 transition-opacity`}
                  whileHover={{ scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                >
                  View Profile
                  <FaExternalLinkAlt className="inline-block ml-2 w-3 h-3" />
                </motion.a>

                {/* Hover Glow Effect */}
                <div className={`absolute -inset-0 bg-gradient-to-r ${profile.color}
                               opacity-0 group-hover:opacity-10 transition-opacity rounded-2xl`} />
              </motion.div>
            </motion.div>
          ))}
        </div>
